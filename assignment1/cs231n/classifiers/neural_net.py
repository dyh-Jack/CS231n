...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    margin2 = np.exp(scores) / exp_sum
    margin2[np.arange(N), y] += -1
    margin2 /= N

    dW2 = out1.T.dot(margin2)
    dW2 += 2*reg * W2
    grads['W2'] = dW2
    grads['b2'] = np.sum(margin2, axis=0)

    margin1 = margin2.dot(W2.T)  # (N, H)
    margin1[out1 <= 0] = 0
